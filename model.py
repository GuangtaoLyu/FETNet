...
class ETSNetModel():
    ...
    def initialize_model(self, path_g=None, path_d=None, train=True):
        ...
        try:
            start_epoch = load_ckpt(path_g, [('generator', self.G)], [('optimizer_G', self.optm_G)])
            if train:
                start_epoch = load_ckpt(path_d, [('discriminator', self.D)], [('optimizer_D', self.optm_D)])
                self.optm_G = optim.Adam(self.G.parameters(), lr=self.lr)
                self.optm_D = optim.Adam(self.D.parameters(), lr=2 * self.lr)
                self.epoch = start_epoch
                print('Model Initialized, epoch: ', start_epoch)
        except:
            self.epoch = 0
    ...
    def train(self, train_loader, save_path, finetune=False, epochs=500):
        ...
        while self.epoch < epochs:
            ...
            if self.epoch % 5 == 0:
                ...
                save_ckpt('{:s}/g_{:d}.pth'.format(save_path, self.epoch), [('generator', self.G)],
                          [('optimizer_G', self.optm_G)], self.epoch)
                save_ckpt('{:s}/d_{:d}.pth'.format(save_path, self.epoch), [('discriminator', self.D)],
                          [('optimizer_D', self.optm_D)], self.epoch)
    ...
    def test(self, test_loader, result_save_path):
        ...
        for gt_images, masks, text, name in test_loader:
            text = text.cuda()
            fake_B, masks_out,mask_1 = self.G(text)
            ...
            if not os.path.exists('{:s}/erase1/'.format(result_save_path)):
                os.makedirs('{:s}/erase1/'.format(result_save_path))
            if not os.path.exists('{:s}/erase2/'.format(result_save_path)):
                os.makedirs('{:s}/erase2/'.format(result_save_path))
            if not os.path.exists('{:s}/mask_1/'.format(result_save_path)):
                os.makedirs('{:s}/mask_1/'.format(result_save_path))
            ...
            for k in range(comp_B.size(0)):
                ...
                file_path = '{:s}/erase1/{:s}.png'.format(result_save_path,  name[k])
                save_image(grid, file_path)
                ...
                grid = make_grid(mask_1[k:k + 1])
                file_path = '{:s}/mask_1/{:s}.png'.format(result_save_path, name[k])
                save_image(1-grid, file_path)
    ...
    def forward(self, text,mask, gt_image):
        ...
        self.mask = mask
        fake_B, fake_mask,x_mask_1 = self.G(text)
        self.fake_mask = fake_mask
        self.fake_mask_2 = x_mask_1
        self.fake_B = fake_B
    ...
    def get_g_loss(self):
        ...
        mask_loss = self.diceLoss(self.mask,self.fake_mask) + 0.01*self.diceLoss(torch.ones_like(self.fake_mask_2),self.fake_mask_2)
